...
# modifications - paths and file names, replaced deprecated make_tag_dictionary w/ make_label_dictionary
def read_in_AMT():
	# define columns
	columns = {0: 'text', 1: 'ner'}

	# this is the folder in which train, test and dev files reside
	data_folder = 'MedRed/data/AMT/labels'

	# init a corpus using column format, data folder and the names of the train, dev and test files
	corpus: Corpus = ColumnCorpus(data_folder, columns,
	                              train_file='NER_Reddit_AMT_labels_href_train.csv',
	                              test_file='NER_Reddit_AMT_labels_href_test.csv',
	                              dev_file='NER_Reddit_AMT_labels_href_dev.csv')
